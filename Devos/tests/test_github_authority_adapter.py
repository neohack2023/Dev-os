...
class FakeGitHub:
    def __init__(self):
        self.branches = {"main": {"protected": True, "commit": {"sha": BASE}}, "devos/candidate": {"protected": False, "commit": {"sha": CANDIDATE}}}
        self.rule_rows = [{"type": "pull_request", "parameters": {"required_approving_review_count": 1}}, {"type": "required_status_checks", "parameters": {"required_status_checks": [{"context": "ci"}]}}]
        self.protection_row = None; self.pr_row = None
        self.review_rows = [{"state": "APPROVED", "user": {"login": "reviewer"}}]
        self.status_row = {"statuses": [{"context": "ci", "state": "success"}]}; self.check_rows = []
        self.merge_response = {"merged": True, "sha": MERGE, "message": "merged"}; self.commit_rows = {MERGE: {"parents": [{"sha": BASE}, {"sha": CANDIDATE}]}}
    ...
    def create_pr(self, repository, head, base, title, body): self.pr_row = {"number": 7, "state": "open", "mergeable": True, "html_url": "https://example/pr/7", "head": {"sha": CANDIDATE}, "base": {"ref": "main"}}; return self.pr_row
...
class GitHubAuthorityAdapterTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(); self.addCleanup(self.temp.cleanup); self.db = Path(self.temp.name) / "authority.db"; self.connection = connect_runtime(self.db); self.addCleanup(self.connection.close); self.client = FakeGitHub(); self._seed_mason()
    def _seed_mason(self, *, outcome="APPLIED"):
        ...
        self.connection.execute("INSERT INTO mason_execution_plans(plan_id,decision_id,envelope_id,target_branch,base_revision,candidate_revision,change_digest,authorization_digest,payload_json,prepared_at,local_execution_authorized,remote_write_authorized) VALUES (?,?,?,?,?,?,?,?,?,?,1,0)",("mason-plan:fixture",decision_id,envelope_id,"main",BASE,CANDIDATE,"d"*64,"e"*64,json.dumps(mason_plan,sort_keys=True,separators=(",",":")),"2026-09-14T00:00:00Z"))
    ...
    def test_schema_v8_and_prepare_binds_policy(self):
        plan=self.prepare(); self.assertEqual(8,CURRENT_SCHEMA_VERSION); self.assertTrue(plan["remote_write_authorized"]); self.assertEqual("merge",plan["merge_method"]); self.assertEqual(["ci"],plan["policy"]["required_checks"]); self.assertEqual(1,plan["policy"]["required_approvals"])
