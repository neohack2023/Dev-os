...
class GitHubAuthorityAdapterTests(unittest.TestCase):
    ...
    def _seed_mason(self, *, outcome="APPLIED"):
        self.connection.execute(
            ...
            ("mason-plan:fixture","decision:fixture","envelope:fixture","main",BASE,CANDIDATE,"d"*64,"e"*64,json.dumps({"plan_id":"mason-plan:fixture","repository":"owner/fixture","target_branch":"main","base_revision":BASE,"candidate_revision":CANDIDATE,"change_digest":"d"*64},sort_keys=True,separators=(",",":")),"2026-09-14T00:00:00Z"),
        )
    ...
    def test_schema_v7_and_prepare_binds_policy(self):
        ...
        self.assertTrue(plan["remote_write_authorized"])
        self.assertEqual("merge", plan["merge_method"])
        self.assertEqual(["ci"], plan["policy"]["required_checks"])
        self.assertEqual(1, plan["policy"]["required_approvals"])

    def test_unprotected_policy_is_rejected(self):
        self.client.branches["main"]["protected"] = False
        self.client.rule_rows = []
        with self.assertRaisesRegex(ValueError, "no active protection"):
            self.prepare()

    def test_linear_history_policy_is_rejected(self):
        self.client.rule_rows.append({"type":"required_linear_history"})
        with self.assertRaisesRegex(ValueError, "unsupported GitHub authority policy"):
            self.prepare()

    def test_remote_candidate_sha_mismatch_is_rejected(self):
        self.client.branches["devos/candidate"]["commit"]["sha"] = "f"*40
        with self.assertRaisesRegex(ValueError, "exact MASON candidate"):
            self.prepare()

    def test_open_pr_is_bound_to_exact_candidate(self):
        plan = self.prepare()
        binding = open_pr(self.connection,plan["plan_id"],self.client,"2026-09-14T00:03:00Z")["pr"]
        self.assertEqual(7,binding["pr_number"])
        self.assertEqual(CANDIDATE,binding["head_revision"])
        replay = open_pr(self.connection,plan["plan_id"],self.client,"later")
        self.assertTrue(replay["replayed"])

    def test_missing_required_check_blocks_ready(self):
        plan = self.prepare(); open_pr(self.connection,plan["plan_id"],self.client,"2026-09-14T00:03:00Z")
        self.client.status_row = {"statuses":[]}
        ready = verify_ready(self.connection,plan["plan_id"],self.client)
        self.assertFalse(ready["ready"])
        self.assertTrue(any("required check" in r for r in ready["reasons"]))

    def test_policy_drift_blocks_ready(self):
        plan = self.prepare(); open_pr(self.connection,plan["plan_id"],self.client,"2026-09-14T00:03:00Z")
        self.client.rule_rows.append({"type":"commit_message_pattern","parameters":{"operator":"starts_with","pattern":"X"}})
        ready = verify_ready(self.connection,plan["plan_id"],self.client)
        self.assertFalse(ready["ready"])
        self.assertIn("GitHub policy changed after preparation",ready["reasons"])

    def test_merge_emits_remote_authority_receipt(self):
        plan = self.prepare(); open_pr(self.connection,plan["plan_id"],self.client,"2026-09-14T00:03:00Z")
        result = merge_remote(self.connection,plan["plan_id"],self.client,"2026-09-14T00:04:00Z")
        receipt = result["receipt"]
        self.assertEqual("MERGED",receipt["outcome"])
        self.assertEqual(MERGE,receipt["authoritative_revision"])
        self.assertTrue(receipt["candidate_is_authoritative_parent"])
        self.assertEqual("GITHUB_TARGET_BRANCH_UPDATED",receipt["remote_authority_effect"])
        self.assertTrue(receipt["remote_write_performed"])
        replay = merge_remote(self.connection,plan["plan_id"],self.client,"later")
        self.assertTrue(replay["replayed"])

    def test_projection_rebuild_style_reconnect_preserves_receipt(self):
        plan = self.prepare(); open_pr(self.connection,plan["plan_id"],self.client,"2026-09-14T00:03:00Z"); merge_remote(self.connection,plan["plan_id"],self.client,"2026-09-14T00:04:00Z")
        self.connection.close(); self.connection = connect_runtime(self.db)
        self.assertEqual(1,self.connection.execute("SELECT count(*) FROM github_authority_receipts").fetchone()[0])
